...
// 1. تجهيز المتغيرات من سلة أو استخدام قيم افتراضية
const waNumber =
  window.waSettings &&
  window.waSettings.number &&
  window.waSettings.number.trim() !== ""
    ? window.waSettings.number.trim()
    : "201069785506";

const rawMessage =
  window.waSettings &&
  window.waSettings.message &&
  window.waSettings.message.trim() !== ""
    ? window.waSettings.message.trim()
...
// 2. جلب الوقت الحالي بصيغة (HH:MM)
