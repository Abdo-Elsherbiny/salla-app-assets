...
// دالة بناء الويدجت
function buildWhatsAppWidget(waNumber, rawMessage) {
  // التأكد من عدم وجود ويدجت قديم
...
function initSallaWidget() {
  // هنتأكد إن كائن سلة موجود وإن الإعدادات تم تحميلها
  if (typeof salla !== "undefined" && salla.config && salla.config.get) {
    let fetchedNumber = salla.config.get("app.whatsapp_number");
    let fetchedMessage = salla.config.get("app.whatsapp_message");

    // لو البيانات لسه ماحملتش من سيرفر سلة، استنى 200 ملي ثانية وجرب تاني
    if (fetchedNumber === undefined || fetchedMessage === undefined) {
      setTimeout(initSallaWidget, 200);
      return;
    }

    console.log("✅ تمت قراءة الإعدادات بنجاح:", fetchedNumber, fetchedMessage);

    // لو الإعدادات فاضية (التاجر ماملهاش)، نحط قيم افتراضية عشان الويدجت ميضربش
    fetchedNumber = fetchedNumber || "201000000000";
    fetchedMessage = fetchedMessage || "مرحباً!";

    buildWhatsAppWidget(fetchedNumber, fetchedMessage);
  } else {
    // لو سلة لسه بتبدأ، استنى وجرب تاني
    setTimeout(initSallaWidget, 200);
  }
}
